...
import org.ruoyi.domain.bo.knowledge.KnowledgeAttachAuditBo;
...
public class KnowledgeAttachServiceImpl implements IKnowledgeAttachService {
    ...
    private LambdaQueryWrapper<KnowledgeAttach> buildQueryWrapper(KnowledgeAttachBo bo) {
        ...
        lqw.like(StringUtils.isNotBlank(bo.getCourseName()), KnowledgeAttach::getCourseName, bo.getCourseName());
        lqw.eq(StringUtils.isNotBlank(bo.getCourseCode()), KnowledgeAttach::getCourseCode, bo.getCourseCode());
        lqw.eq(StringUtils.isNotBlank(bo.getMaterialType()), KnowledgeAttach::getMaterialType, bo.getMaterialType());
        lqw.eq(StringUtils.isNotBlank(bo.getTerm()), KnowledgeAttach::getTerm, bo.getTerm());
        lqw.like(StringUtils.isNotBlank(bo.getTags()), KnowledgeAttach::getTags, bo.getTags());
        lqw.eq(bo.getAuditStatus() != null, KnowledgeAttach::getAuditStatus, bo.getAuditStatus());
        return lqw;
    }
    ...
    public void upload(KnowledgeInfoUploadBo bo) {
        ...
        knowledgeAttach.setAuditStatus(0);
        knowledgeAttach.setCourseName(bo.getCourseName());
        knowledgeAttach.setCourseCode(bo.getCourseCode());
        knowledgeAttach.setMaterialType(bo.getMaterialType());
        knowledgeAttach.setTerm(bo.getTerm());
        knowledgeAttach.setTags(bo.getTags());

        baseMapper.insert(knowledgeAttach);

        if (!Boolean.FALSE.equals(bo.getAutoParse())) {
    ...
    public void parse(Long id) {
        ...
        try {
            ...
            try (InputStream inputStream = ossService.getObjectContent(attach.getOssId())) {
    ...
    @Override
    public Boolean audit(Long id, KnowledgeAttachAuditBo bo) {
        if (id == null || bo == null || bo.getAuditStatus() == null) {
            return false;
        }
        if (bo.getAuditStatus() != 1 && bo.getAuditStatus() != 2) {
            throw new IllegalArgumentException("审核状态只能为通过或驳回");
        }
        KnowledgeAttach attach = baseMapper.selectById(id);
        if (attach == null) {
            return false;
        }
        attach.setAuditStatus(bo.getAuditStatus());
        attach.setRejectReason(bo.getAuditStatus() == 2 ? bo.getRejectReason() : null);
        attach.setAuditBy(LoginHelper.getUserId());
        attach.setAuditTime(new Date());
        return baseMapper.updateById(attach) > 0;
    }
}
