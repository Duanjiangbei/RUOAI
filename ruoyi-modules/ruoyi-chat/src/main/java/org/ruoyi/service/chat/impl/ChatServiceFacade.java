...
public class ChatServiceFacade implements IChatService {
    ...
    private List<ChatMessage> buildContextMessages(ChatRequest chatRequest) {
        ...
        if (chatRequest.getKnowledgeId() != null) {
            ...
            if (knowledgeInfoVo != null) {
                ...
                if (chatModel != null) {
                    log.info("Run manual RAG retrieval: kid={}", chatRequest.getKnowledgeId());

                    QueryVectorBo queryVectorBo = buildQueryVectorBo(chatRequest, knowledgeInfoVo, chatModel);
                    List<KnowledgeRetrievalVo> retrieved = knowledgeRetrievalService.retrieve(queryVectorBo);
                    ...
                    for (KnowledgeRetrievalVo item : retrieved) {
                        if (item != null && StringUtils.isNotBlank(item.getContent())) {
                            retrievedTexts.add(item.getContent());
                        }
                    }
                    chatRequest.setCitationJson(JsonUtils.toJsonString(retrieved));
                    log.info("Manual RAG retrieved {} fragment(s) for kid={}",
                        retrievedTexts.size(), chatRequest.getKnowledgeId());

                    if (!retrievedTexts.isEmpty()) {
                        userMessage = UserMessage.userMessage(buildKnowledgePrompt(chatRequest.getContent(), retrievedTexts));
                    } else {
                        log.warn("No knowledge fragment retrieved for kid={}", chatRequest.getKnowledgeId());
                    }
                }
            }
        }
    ...
    private QueryVectorBo buildQueryVectorBo(ChatRequest chatRequest, KnowledgeInfoVo knowledgeInfoVo,
        ...
        queryVectorBo.setEnableHybrid(knowledgeInfoVo.getEnableHybrid() != null && knowledgeInfoVo.getEnableHybrid() == 1);
    ...
    private String buildKnowledgePrompt(String question, List<String> fragments) {
        StringBuilder prompt = new StringBuilder();
        prompt.append("You are a campus course material question-answering assistant. ");
        prompt.append("Answer in Chinese. Use the following knowledge base snippets as the primary source. ");
        prompt.append("If the snippets do not contain the answer, say the knowledge base has no relevant answer and do not invent facts.\n\n");
        prompt.append("Knowledge base snippets:\n");
        for (int i = 0; i < fragments.size(); i++) {
            prompt.append("[").append(i + 1).append("]\n");
            prompt.append(fragments.get(i)).append("\n\n");
        }
        prompt.append("User question:\n").append(question).append("\n\n");
        prompt.append("Answer requirements: be concise, cite snippet numbers such as [1] when useful, and focus on the course material.");
            ...
            public void onCompleteResponse(ChatResponse completeResponse) {
                try {
                    ...
                    if (fullMessage.isEmpty()) {
                        ...
                        // 保存助手回复消息
                        chatMessageService.saveChatMessage(userId, chatRequest.getSessionId(), fullMessage, RoleType.ASSISTANT.getName(), chatRequest.getModel(), chatRequest.getCitationJson());
                    }
