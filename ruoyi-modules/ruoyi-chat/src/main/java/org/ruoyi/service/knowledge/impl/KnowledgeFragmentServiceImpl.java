...
public class KnowledgeFragmentServiceImpl implements IKnowledgeFragmentService {
    ...
    private LambdaQueryWrapper<KnowledgeFragment> buildQueryWrapper(KnowledgeFragmentBo bo) {
        ...
        lqw.isNotNull(KnowledgeFragment::getKnowledgeId);
        if (bo.getAttachId() != null) {
            KnowledgeAttach attach = knowledgeAttachMapper.selectById(bo.getAttachId());
            if (attach == null || StringUtils.isBlank(attach.getDocId())) {
                lqw.eq(KnowledgeFragment::getId, -1L);
                return lqw;
            }
            lqw.eq(KnowledgeFragment::getDocId, attach.getDocId());
            lqw.eq(KnowledgeFragment::getKnowledgeId, attach.getKnowledgeId());
        }
